...
/// Records a received Orchard-domain note in the received-notes table identified by
/// `table_prefix`.
///
...
pub(crate) fn mark_orchard_note_spent(
    ...
    mark_note_spent(conn, ORCHARD_TABLES_PREFIX, "Orchard", tx_ref, nf)
}
...
pub(crate) fn mark_ironwood_note_spent(
    ...
    mark_note_spent(conn, IRONWOOD_TABLES_PREFIX, "Ironwood", tx_ref, nf)
}

/// Marks the received note in `{table_prefix}_received_notes` with the given nullifier as spent in
/// the referenced transaction. `pool_name` is used only for error reporting.
fn mark_note_spent(
    conn: &Connection,
    table_prefix: &str,
    pool_name: &str,
